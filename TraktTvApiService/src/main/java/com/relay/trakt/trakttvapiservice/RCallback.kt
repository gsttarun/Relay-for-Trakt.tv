...
class RCallback<T> {
    ...
    private var failure: (T?, message: String?, throwable: Throwable?) -> Unit = { _, _, _ -> }

    fun onSuccess(init: (T?, message: String?) -> Unit) {
        success = init
    }

    fun onFailure(init: (T?, message: String?, throwable: Throwable?) -> Unit) {
        failure = init
    }
        ...
        fun <T> getCallback(init: RCallback<T>.() -> Unit): Callback<T> {
            ...
            return object : Callback<T> {
                override fun onFailure(call: Call<T>, t: Throwable) {
                    rCallback.failure(null, t.message!!, t)
                }
                ...
                override fun onResponse(call: Call<T>, response: Response<T>) {
                    if (response.isSuccessful) {
                        ...
                        rCallback.failure(response.body(), response.message(), null)
                    }
                }
            }
        }
